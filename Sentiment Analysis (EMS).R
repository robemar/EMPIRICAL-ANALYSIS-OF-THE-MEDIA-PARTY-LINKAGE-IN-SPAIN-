...
load("D:/Master Denmark/3rd Semester/Political Data Science/Final Exam/True Final Exam/1. Downloading/SanchezElMundotopics.RData")
...
save(text.df, file="itsworking(EMS).RData")
...
qplot(data = newdata, x = score, 
      geom = "bar", xlab = "", 
      ylab = "Frequency", main = "Sanchez's Executive teatment ElMundo") + 
  theme_bw() +
...
save(newdata, file="SentientAnalysis(EMS).RData")
load("SentientAnalysis(EMS).RData")
...
save(dataEMS, file="Complete(EMS).RData")
