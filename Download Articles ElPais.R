...
base.url <- "https://elpais.com/hemeroteca/elpais/"

# create main archive
if(!dir.exists("ElPaisarticles")) dir.create("ElPaisarticles")

# create sub-archive for search results
archive.folder <- "./ElPaisarticles/search_archive/"
  ...
  for (mo in 2:9) { 
    ...
    if (mo == 2) {
      if (yr %in% c(2008, 2012, 2016)) {
        # unnecessary but undisturbing element from the original code
        mo.days <- 29
      } else {
        # normal years
        mo.days <- 28
    ...
    for (dy in 1:mo.days) { #loop over selected number of days
      ...
      for (tod in c("m", "t", "n")) {
        # build url to fetch
        final.url <- paste0(base.url, yr, "/", mo, "/", dy, "/", tod, "/portada.html")
