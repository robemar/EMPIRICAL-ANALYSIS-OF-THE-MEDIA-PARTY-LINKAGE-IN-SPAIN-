...
load("D:/Master Denmark/3rd Semester/Political Data Science/Final Exam/True Final Exam/1. Downloading/SanchezElPaistopics.RData")
...
save(text.df, file="itsworking(EPS).RData")
...
qplot(data = newdata, x = score, 
      geom = "bar", xlab = "", 
      ylab = "Frequency", main = "Sanchez's Executive teatment ElPais") + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90))



#without_noneutral <- newdata[newdata$sentiment >= -1 & newdata$sentiment <= 1,]
...
ggplot(newdata, aes(x=topic_lab, y=sentiment, label=topic_lab)) + 
  ...
  labs(subtitle="Articles from El Pais", 
       title= "Topic Sentiment of Sanchez's Executive") + 
  coord_flip()


save(newdata, file="SentientAnalysis(EPS).RData")
load("SentientAnalysis(EPS).RData")
...
save(dataEPS, file="Complete(EPS).RData")
