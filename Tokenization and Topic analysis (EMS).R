...
# load article data
load("D:/Master Denmark/3rd Semester/Political Data Science/Final Exam/True Final Exam/1. Downloading/Sanchez(ElMundo)clean.RData")
...
save(metrics, file = "ElMundoSanchez_perplexity.RData")


qplot(data = metrics, x = topics, y = perplexity, geom = "line",
      xlab = "Number of topics",
      ylab = "Perplexity on test data El Mundo") + theme_bw()


# Execute lda with 12 topics (optimal)
lda <- LDA(dtm, k = 12)
save(lda, file = "ElMundoSanchezOptTop.RData")

# Output
terms(lda, 10)

load("ElMundoSanchezOptTop.RData")
...
newdata$topic_lab <- factor(newdata$topic,
                        levels = 1:12,
                        labels = c("Ministry of Finance", "Immigration", "Catalonia", "Spanish Ministries", "Refugee Crisis", "Jailing of Catalan Leaders", 
                                   "Spanish Government", "Titulation Scandals", "Franco", "Spanish Economy", "Censorship Vote",  
                                   "Budget Vote"))

# Frequency plot with specific topcis
qplot(data = newdata, x = topic_lab, 
      geom = "bar", xlab = "", 
      ylab = "Frequency", main = "Sanchez's Executive topic frequency ElMundo") + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90))

terms (lda, 40)

save(newdata, file="SanchezElMundotopics.RData")
