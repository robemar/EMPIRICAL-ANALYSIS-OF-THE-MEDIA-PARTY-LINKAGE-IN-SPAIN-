...
# load article data
load("D:/Master Denmark/3rd Semester/Political Data Science/Final Exam/True Final Exam/1. Downloading/Rajoy(ElMundo)clean.RData")


# Drop articles missing text (don't work?)
newdata <- textclean[!is.na(textclean$full_text), ]


# Create a date
newdata$date <- as.Date(newdata$file.date, "%Y-%m-%d")
...
save(metrics, file = "ElMundoRajoy_perplexity.RData")
...
# Execute lda with 15 topics (optimal)
lda <- LDA(dtm, k = 15)
save(lda, file = "ElMundoRajoyOptTop.RData")

load("ElMundoRajoyOptTop.RData")
...
newdata$topic_lab <- factor(newdata$topic,
                        levels = 1:15,
                        labels = c("Judicial System", "Spanish Politics", "Ministry of Finance", "Catalonia", "Spanish Government", "Catalan Conflict", 
                                   "Spanish Economy", "Catalan Government", "Terrorism", "European Politics", "Censorship Vote",  
                                   "Budget Vote", "Spanish Government", "European Economy", "Catalan Education System"))

# Frequency plot with specific topcis
qplot(data = newdata, x = topic_lab, 
      geom = "bar", xlab = "", 
      ylab = "Frequency", main = "Rajoy's Executive topic frequency ElMundo") + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90))

terms (lda, 40)

save(newdata, file="RajoyElMundotopics.RData")
load("RajoyElMundotopics.RData")
